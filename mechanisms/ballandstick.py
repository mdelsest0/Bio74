...
class BallAndStick(Cell):
    ...
    def _setup_morphology(self,dend_diam0=5,dend_diam1=1,axon_diam0=5,axon_diam1=1):
        ...
        # axon geometry
        self.axon.L = 1000                        
        self.axon.nseg = 1 + 2*int(self.axon.L/40) # 1 segments per 20 µm 
        for seg in self.axon:
            seg.diam = np.interp(seg.x, [0, 1], [axon_diam0, axon_diam1]) # taper from 5 to 1 µm
    ...
    def _setup_recordings(self):
        ...
        self.axon_vs = [h.Vector().record(seg._ref_v) for seg in self.axon] # Record axonal voltages
        self.axon_ims = [h.Vector().record(seg._ref_i_membrane) for seg in self.axon] # Record axonal voltages
